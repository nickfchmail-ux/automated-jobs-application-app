...
import CoverLetterGenerationCard from "@/components/CoverLetterGenerationCard";
import JobStateProvider from "@/components/JobStateProvider";
import LiveFitChip from "@/components/LiveFitChip";
...
export default async function JobDetailLayout({
  ...
  const isEvaluated = job.fit_score !== null && job.fit_score !== undefined;
  ...
  return (
    <JobStateProvider
      ...
      jobId={job.id}
      initialState={{
        resumeStatus: (job.resume_status as ResumeStatus | null) ?? null,
        resumeUrl: (job.resume_url as string | null) ?? null,
        coverLetterStatus:
          (job.cover_letter_status as CoverLetterStatus | null) ?? null,
        coverLetter: (job.cover_letter as string | null) ?? null,
        fit: (job.fit as boolean | null) ?? null,
        fitScore: (job.fit_score as number | null) ?? null,
      }}
    >
      <div className="min-h-screen bg-[var(--paper)] dark:bg-zinc-950">
        ...
        <main className="max-w-5xl mx-auto px-4 sm:px-8 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* Left sidebar — live document generation + skills */}
          <aside className="flex flex-col gap-6">
            {/* Tailored resume — on-demand, independent of evaluation */}
            ...
            {/* Cover letter — on-demand, independent of evaluation */}
            <CoverLetterGenerationCard
              jobId={job.id}
              title={job.title}
              company={job.company}
            />
              ...
              <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {parsedSkills.map((skill, i) => (
                    <span
                      key={i}
                      className="text-xs px-2.5 py-1 rounded-full text-gray-700 dark:text-blue-300"
                    >
                      {skill}
                    </span>
                  ))}
          ...
          {/* Main column — tabs + analysis/details */}
