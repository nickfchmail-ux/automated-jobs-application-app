...
/**
 * Server component that loads the dashboard's stat-heavy sections:
 * fit/not-fit counts, the application tracker, and recently scraped jobs.
 *
 * Kept separate from the page shell and rendered inside a <Suspense> so the
 * core Search → Match → RunHistory UI paints immediately while these
 * count/storage queries stream in — the user never stares at a bare
 * "Loading dashboard…" while waiting on Supabase.
 */
export default async function DashboardStats() {
  ...
  try {
    const [fit, notFit, applied, recents] = await Promise.all([
      supabase
        .from("jobs")
    ...
    if (fit.error)
      console.error("[DashboardStats] fit query error:", fit.error);
    if (notFit.error)
      console.error("[DashboardStats] notFit query error:", notFit.error);
    if (applied.error)
      console.error("[DashboardStats] applied query error:", applied.error);
    if (recents.error)
      console.error("[DashboardStats] recent query error:", recents.error);

    fitCount = fit.count ?? 0;
    notFitCount = notFit.count ?? 0;
    appliedCount = applied.count ?? 0;
    recent = (recents.data as Job[]) ?? [];
  } catch (err) {
    console.error("[DashboardStats] Unexpected error:", err);
  }

  const applied = appliedCount;
  const total = fitCount;

  return (
    <>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base font-semibold text-zinc-700 dark:text-zinc-300 font-display">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        >
          ...
          <p className="text-4xl font-bold text-zinc-900 dark:text-zinc-50 tabular-nums">
            {fitCount}
          </p>
        ...
        <Link
          href="/not-fit"
          className="group block rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm hover:shadow-md hover:border-rose-300 dark:hover:border-rose-700 transition-all"
        >
          <div className="flex items-center justify-between mb-4">
            <div className="w-11 h-11 rounded-xl bg-rose-50 dark:bg-rose-950 flex items-center justify-center">
              <svg
            ...
            <svg
              className="w-4 h-4 text-zinc-300 group-hover:text-rose-400 transition-colors"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={2}
