...
export default function TransparentButton({
  ...
  const colorMap: Record<colors, string> = {
    blue: "border-violet-200 dark:border-violet-800 text-violet-700 dark:text-violet-300 hover:bg-violet-50 dark:hover:bg-violet-950/60",
  ...
  const activeMap: Record<colors, string> = {
    blue: "border-violet-400 dark:border-violet-600 bg-violet-100 dark:bg-violet-900/60 text-violet-700 dark:text-violet-300",
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`inline-flex items-center gap-2 text-sm font-medium px-3.5 py-2 rounded-xl shadow-sm ${noBorder ? "" : "border"} transition-all hover:shadow focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:ring-offset-1 ${disabled ? "border-zinc-200 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-800 text-zinc-400 dark:text-zinc-600 shadow-none cursor-not-allowed" : colorStyle}`}
    >
