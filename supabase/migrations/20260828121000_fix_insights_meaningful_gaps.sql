-- ============================================================
--  FIX: get_user_insights — meaningful strengths/gaps.
--
--  The previous SQL only dropped phrases that were EXACTLY a
--  single stopword (`term ~* '^(the|...)$'`), which let junk like
--  "no experience", "the job", "lacks the" leak through as gaps.
--
--  The original frontend logic (lib/insights.ts `tokenize`)
--  matched capitalized tech noun-phrases and dropped any phrase
--  whose FIRST or LAST word is a stopword. This rewrite ports that
--  faithfully:
--    1. regexp_matches → capitalized phrase (+ optional 2nd word)
--    2. split into first_w / last_w
--    3. keep only if NEITHER first_w NOR last_w is a stopword
--       AND first word is ≥2 chars
--
--  Result: gaps become real missing skills (Java, C#, Flutter,
--  Spring Boot, React, Python, AWS…) instead of "no experience".
--  Strengths get the same fix (identical logic, both directions).
-- ============================================================
...
begin
  ...
  with stopwords as (
    select unnest(array[
      ...
      'background','demonstrated','demonstrates','proven'
    ]) as word
  ),
  j as (
    select
