-- ============================================================
--  get_user_insights(p_user_id uuid) → jsonb
--
--  THE dashboard aggregation. Replaces the frontend's "fetch up to
--  5000 jobs × 22 columns and aggregate in Node" (lib/insights.ts)
--  with a SINGLE SQL pass that returns the computed insight as a
--  few KB of JSON.
--
--  Why this is the right fix:
--    - Payload: ~MBs of raw rows → ~KB of computed insight.
--    - Accuracy: the old `.limit(5000)` silently truncated insight
--      for power users; SQL aggregates over ALL rows.
--    - Index: uses idx_jobs_user_created_at (user_id, created_at DESC).
--    - RLS: SECURITY DEFINER + explicit p_user_id param (service-role
--      callers bypass RLS; we NEVER rely on auth.uid()).
--
--  The output shape mirrors lib/insights.ts Insights so the frontend
--  call site stays byte-identical. Ported logic:
--    - strengths/gaps: tokenize fit_reasons/not_fit_reasons, keep
--      capitalized tech noun-phrases, drop stopwords, top N by count.
--    - salary: normalize to monthly HKD (year→/12, hour→×160).
--    - trend: group by pipeline_run_id, oldest→newest, up to 8 runs.
-- ============================================================
...
begin
  ...
  -- ── Strengths: capitalized tech noun-phrases from fit_reasons ──
  strength_tokens as (
    select lower(t[1]) as term
    from matches m,
  ...
  strength_grouped as (
    ...
    where term is not null
      and length(term) >= 2
      and not (term ~* '^(the|and|with|for|in|of|no|a|an|to|on|is|are|as|or|at|be|not|which|that|this|their|his|her|its|experience|expertise|skills|skill|knowledge|strong|good|working|hands|related|proficiency|proficient|relevant|years|year|role|jobs|job|work|development|developer|engineer|requirement|requirements|candidate|position|company|team|ability|background|demonstrated|demonstrates|proven)$')
    group by term
    order by cnt desc
    limit 6
  ),
  -- ── Gaps: same treatment on not_fit_reasons ──
  gap_tokens as (
    select lower(t[1]) as term
    from notfit n,
...
-- ── Grants (follow increment_run_board convention) ─────────────
