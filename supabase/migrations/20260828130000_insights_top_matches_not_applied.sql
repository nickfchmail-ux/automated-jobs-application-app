-- ============================================================
--  get_user_insights — "Your best matches" = only NOT-APPLIED
--
--  The overview's top-matches section should surface the
--  highest-scoring roles the user hasn't applied to yet (actionable:
--  "what to apply to next"), not roles they've already applied to.
--  Re-applies the function with `where applied = false` in the
--  `top_matches` CTE.
-- ============================================================
...
begin
  ...
  -- ── Top matches: highest-scoring roles NOT yet applied to ──
  -- `applied is not true` (not `applied = false`) because `applied` is
  -- nullable — a NULL means "not applied" and must be included.
  top_matches as (
    select
